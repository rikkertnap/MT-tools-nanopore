...
def make_iondistribution():
    ...
    for i in range(len(pHval)):
        ...
        # write to vtk file 
        fnamevtk=xsolnames[i].replace("avsol","cNa",1)
        write_numpy_to_vtkfile(np.array(cion.Na),fnamevtk,delta) 
        # write to vtk file 
        fnamevtk=xsolnames[i].replace("avsol","cCl",1)
        write_numpy_to_vtkfile(np.array(cion.Cl),fnamevtk,delta)
