...
def get_pH_csalt_value(sysname):
    """ 
    obtains pH value for system files returned in a sorted array
    """
    fname=sysname
    file=open(fname,'r')
...
def make_pHdistribution_vtk(systemfile):
    ...
    pHval, csaltval = get_pH_csalt_value(systemfile)

    # get avsol*.vtk and frdis*.vtk file
    xsolname=systemfile.replace("system","avsol")
    xsolname=xsolname.replace("dat","vtk")
    potname=xsolname.replace("avsol","poten")
    ...
    xbulk = init_xbulk(pHval,csaltval)
    expmu = init_expmu(xbulk)
    # get pi and psi 
    xsolname, xsol = get_data_from_vtk(xsolname)
    potname, pot = get_data_from_vtk(potname)
    ...
    # write to vtk file 
    fnamevtk=xsolname.replace("avsol","pH",1)
...
def main():
    ...
    match args.filetype:
        case "vtk":
            ...
            name ="pH"
        case _:
            raise ValueError("filetype: dat not implemented yet")
